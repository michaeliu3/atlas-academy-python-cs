...
| Visual clarity, accessibility, keyboard/screen-reader/performance checks | In progress | Existing visual studios and reader; Mermaid retains safe structural labels and an honest technical-source fallback | Add automated accessibility/browser checks, per-diagram concise prose equivalents, performance budgets, and retrofit earlier modules. |
...
| Strict TypeScript, dependency risk triage, bounded lab truth | In progress | `wrangler.jsonc`, generated Worker types, strict local typecheck, CI gate | Verify the GitHub CI run, document bounded OS/process labs accurately, and triage alerts with evidence. |
