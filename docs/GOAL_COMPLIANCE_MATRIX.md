...
| Auditable GitHub releases and preserved change history | In progress | `main` and the active review branch reject force-pushes/deletion, require linear history, and apply those rules to administrators; additive Git commits remain on the review branch; `docs/RELEASE_PROVENANCE.md` records historical GitHub Release/tag facts and failed browser candidates separately from release success | Keep protection settings under review, make browser-accessibility acceptance a required `main` check before treating branch policy as a full release gate, maintain additive commits and reviewable PRs, record a successful candidate with its exact checkout/ref distinction, and verify private deployment separately. |
