...
| Auditable GitHub releases and preserved change history | In progress | `main` and the active review branch reject force-pushes/deletion and require linear history; additive Git commits remain on the review branch; `docs/RELEASE_PROVENANCE.md` records historical GitHub Release/tag facts and both failed browser candidates separately from release success | Keep protection settings under review, maintain additive commits and reviewable PRs, record a successful candidate with its exact checkout/ref distinction, and verify private deployment separately. |
