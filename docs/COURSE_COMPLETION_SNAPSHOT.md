...
| Guided Codex learning | Separate reusable TA and Study Partner packages, module contexts, whiteboard protocol, and supportive oral-defense flow exist. Copyable distinct role handoffs and a concise record-control addendum exist: explicit `records on` before a note, visible chat-level acknowledgement, a save locator only after evidence, and a manual delete/archive fallback. The private route starts the advanced packs directly, without a pilot gate. On 2026-08-04, the two designated live-capable chats received and acknowledged their current material-first role briefs. | Operational instructions are complete; actual on-screen rendering, Live quality/voice, record activation, Notion write, and deletion behavior remain platform behavior to observe when the learner chooses to use those chats, not material-readiness prerequisites. |
...
| Privacy, safety, build, and Git history | Existing local-first, sanitization, build/type/lint, and Git protections remain in place; [Course CI run 30741679780](https://github.com/michaeliu3/atlas-academy-python-cs/actions/runs/30741679780) recorded source head `2bccea0…` through the portal, Python 3.12/3.14, and browser-accessibility jobs. Current alert facts and candidate remediation are in the [dependency risk register](DEPENDENCY_RISK_REGISTER.md). Local course history remains additive, GitHub authentication is available, and the integrated history is mirrored to the reviewable `codex/atlas-60-day-route-history` source ref; current remote provenance verification remains pending. | Protected-default-branch alert recalculation, the remaining Drizzle/esbuild path, release-grade hardening, and a normal post-push provenance check remain pending. Use only additive commits and normal pushes; the CI result is not a release, deployment, security-clean, or complete-accessibility claim. |
...
4. Conduct the final requirement-by-requirement audit against the chosen
