...
| Privacy, safety, build, and Git history | Existing local-first, sanitization, build/type/lint, and Git protections remain in place; [Course CI run 30741679780](https://github.com/michaeliu3/atlas-academy-python-cs/actions/runs/30741679780) recorded source head `2bccea0…` through the portal, Python 3.12/3.14, and browser-accessibility jobs. Current alert facts and candidate remediation are in the [dependency risk register](DEPENDENCY_RISK_REGISTER.md). Local course history remains additive, but GitHub authentication is currently invalid, so remote push and current remote provenance verification remain pending re-authentication. | Protected-default-branch alert recalculation, the remaining Drizzle/esbuild path, release-grade hardening, and a normal post-authentication push remain pending. Use only additive commits and normal pushes; the CI result is not a release, deployment, security-clean, or complete-accessibility claim. |
