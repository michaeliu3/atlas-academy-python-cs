...
| Notion learning record | Local-first portal plus a conditional designated-chat concise-note policy, privacy controls, templates, and existing Notion hub. On 2026-08-01, one text-only QA in each designated chat completed a non-sensitive `QA — safe to delete` note creation; the TA then inserted and re-fetched one correction on its own QA page. First automatic recording now requires `records on` in that exact chat; the role addendum treats controls as chat-level intent rather than proof of a write and supplies a manual delete/archive fallback. | Creation and correction are observed for a non-sensitive QA page. The available integration exposed no archive/delete action; `pause records`, first-time activation, save locators, and manual fallback are now instructed in-chat but their effects have not been observed. Visual rendering and Live voice behavior remain unverified; no raw transcripts or sensitive data should be stored. |
