...
| Guided Codex learning | Separate reusable TA and Study Partner packages, module contexts, whiteboard protocol, and supportive oral-defense flow exist. Copyable distinct role handoffs and a concise record-control addendum exist: explicit `records on` before a note, visible chat-level acknowledgement, a save locator only after evidence, and a manual delete/archive fallback. Seed and confirm the matching handoff in each exact current designated chat before relying on it. The portal and private route surface the first-time `records on` consent condition. | Operational instructions are complete; actual Live quality, voice, math rendering, text fallback, record activation, and deletion behavior remain platform behavior to observe in the exact chats. |
