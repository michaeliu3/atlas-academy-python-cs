...
| M32 asynchronous execution and reproducibility | [JAX asynchronous dispatch](https://docs.jax.dev/en/latest/async_dispatch.html) and [PyTorch reproducibility](https://docs.pytorch.org/docs/main/notes/randomness.html) | Submission versus readiness/observation and the non-portability of same-seed claims across releases, platforms, and CPU/GPU paths. | Project documentation changes with releases. Link-only/original explanations; pin version, backend, device, and settings before an execution claim. |
...
| **M32 reproducibility dossier.** Session 6 records source revision, packages, runtime, backend, OS/device/driver, data identity, timing boundary, raw observations, and a non-claim. | MIT 12.010 pairs verification with documentation/version-control/reproducible practice. PyTorch expressly limits cross-release, cross-platform, and CPU/GPU reproducibility even with identical seeds. | **Aligned with a small evidence-design gap.** The dossier can record an environment, but it does not yet require a deliberate changed-environment comparison that makes the portability boundary observable. |
