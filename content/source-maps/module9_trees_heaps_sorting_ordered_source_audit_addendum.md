...
| **P02** | Python Software Foundation: [`bisect`](https://docs.python.org/3.14/library/bisect.html) | Insertion-point partition behavior, lower/upper boundary distinction, `key=`, and the distinction between logarithmic search and list-insertion movement. It is not a thread-safety or full ordered-index contract. | Accessed 2026-08-02; rechecked 2026-08-03. Link-and-paraphrase only; recheck terms before exact reuse. |
...
| Sorted arrays and heaps; Session 4 | `bisect` lower/upper partition behavior, a lower-bound loop invariant and progress argument, list movement distinction, heap invariant, parent/child repair, heapify model, and bounded source reading | `P01`, `P02`, `C01`; original Atlas proof sketch | A heap only orders ancestors against children and a `bisect` search does not make list insertion logarithmic. A proof sketch assumes the input is sorted under the same comparison; CPython helper shape must not become an Atlas public API. |
