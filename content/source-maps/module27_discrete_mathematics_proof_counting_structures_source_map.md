...
| 3. Count, decompose, and bound | “Which objects are we counting, and why is every object counted exactly once?” | Addition/product rules, bijections, inclusion–exclusion, pigeonhole, recurrence/base cases, `O/Ω/Θ`. | S01 + S03 + S07; S10 code trace. | Learner labels the sample space, compares a `math.comb` value with enumerated positions, derives a recurrence, and names the finite-experiment boundary. |
| 4. Encode a sequence; reason modulo a relation | “Can an algebraic representation preserve the combinatorial structure we need?” | Ordinary generating functions; divisibility, gcd, congruence, modular inverse. | S03 + S09 + S01/S02; S10. | Visual coefficient table and accessible text equivalent. Learner checks coefficient claims in a small finite case, then proves a modular step and tests `pow` only after checking the gcd condition. |
| 5. Relationships at scale | “Which connection pattern is guaranteed, and which merely looks plausible?” | Graphs/trees/connectivity, bipartite graphs/matchings, relation graphs. | S01/S02 + S03 + S07. | Read an adjacency structure, make a BFS/DFS prediction, build an alternating-path explanation, and refute a greedy matching claim with a minimal graph. |
| 6. Partial information and mathematical defense | “When does ‘before’ form an order, and when do choices have a join/meet?” | Partial orders, Hasse diagrams, lattices; capstone proof/counterexample/oral defense. | S05 + S08 + S01; S14 handoff. | Learner derives a Hasse diagram from pair facts, finds a missing join, then orally defends one proof, one counterexample, one trace, and one implementation boundary. |
