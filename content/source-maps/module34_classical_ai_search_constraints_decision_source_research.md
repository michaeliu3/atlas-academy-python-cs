...
| S34-08 | UC Berkeley CS188, [Decision Networks](https://inst.eecs.berkeley.edu/~cs188/textbook/vpis/decision-networks.html) | UC Berkeley course staff | Official learner-facing route for conditional expected-utility calculations under named state, evidence, action, utility, and probability assumptions. | University course material, accessed 2026-08-03. Link-only/original finite examples; do not copy prose, figures, exercises, or code. Historical expected-utility sources remain citation background, not the formula-reading route. |
...
| S34-20 | UC Berkeley CS188, [logic and first-order inference](https://inst.eecs.berkeley.edu/~cs188/textbook/logic/), [Bayesian networks](https://inst.eecs.berkeley.edu/~cs188/textbook/bayes-nets/), and [hidden Markov models](https://inst.eecs.berkeley.edu/~cs188/textbook/hmms/) | Berkeley course staff; official course textbook routes | Calibrates FOL terms/quantifiers, clause-based inference, Bayes-net factorization, and HMM filtering as distinct formal objects. | Accessed 2026-08-04. Course material is link-only/original paraphrase; no copied proofs, figures, exercises, or code. |
| S34-21 | MIT OpenCourseWare, [6.825 POMDP lecture](https://ocw.mit.edu/courses/6-825-techniques-in-artificial-intelligence-sma-5504-fall-2002/47a24e96943c8ee02a774dc52f300e29_Lecture20FinalPart1.pdf) and [lecture-notes index](https://ocw.mit.edu/courses/6-825-techniques-in-artificial-intelligence-sma-5504-fall-2002/pages/lecture-notes/) | MIT course staff; official graduate AI course | Calibrates the POMDP extension: hidden state, observation model, belief-state policy, and its computational boundary beyond an HMM/MDP. | Accessed 2026-08-04. MIT OCW notices apply; link-only/original notation and bounded belief-update card. |
| S34-22 | MIT OpenCourseWare, [6.S890 Topics in Multiagent Learning lecture notes](https://ocw.mit.edu/courses/6-s890-topics-in-multiagent-learning-fall-2024/resources/lecture-notes/) and Berkeley CS188 [general games](https://inst.eecs.berkeley.edu/~cs188/textbook/games/general.html) | MIT/UC Berkeley course staff; official university routes | Calibrates zero-sum versus general-sum payoffs, best response, Nash-equilibrium language, and the boundary between strategic reasoning and single-agent expected utility. | Accessed 2026-08-04. Link-only/original payoff tables; no copied lecture assets or equilibrium claims beyond the declared finite game. |
...
| Closed-world versus missing-fact semantics | S34-19 makes the planning course's closed-world premise visible beside execution and learning under uncertainty. | An original card must distinguish model-false, missing observation, and no state variable; it must add an observation/belief boundary or withdraw the claim rather than silently infer falsehood. |
...
| State/observation, A-star, AC-3, and Bellman boundary cards | S34-18 provides direct official routes for informed-search conditions, propagation/requeue reasoning, and finite MDP structure. | Original cards must show the exact countermodel or trace and their non-claim; they must not copy course material or become a general sensor, search, CSP, model-checking, or MDP implementation. |
| FOL/resolution, Bayesian networks, and HMM filtering | S34-20 provides official routes for logic syntax/inference and probabilistic factorization/filtering. | Original cards must expose scope, substitution, conditional-independence, transition/emission, and bounded-search boundaries; they do not constitute a theorem prover or calibrated inference engine. |
| POMDP belief policies and game theory | S34-21–S34-22 provide official routes for hidden-state belief updates, strategic payoffs, and equilibrium vocabulary. | Use original finite belief/payoff cards; do not claim a deployed policy, general equilibrium solver, fairness, or authority. |
...
| --- | --- | --- | --- | --- |
| M34-C01 | A deterministic state-space model can be written as \(P=(S,A,T,s_0,G,c,O)\): states, actions, transition relation/function, initial state, goal predicate, path-cost rule, and observation model. | Specify whether the transition is deterministic/partial, whether action cost is additive, which facts are world-state facts versus observations, whether a belief/update model is needed, the representation’s granularity, and which real-world factors are outside the state. | Derive two non-isomorphic state encodings for the same story, then trace how each changes legal actions and repeated-state detection. Check one finite countermodel before treating an observation implication as a state fact. | A map location alone is not a state if key possession, fuel, time, or a safety condition changes future legal actions. A lit signal can be compatible with an absent key. A formal state is not evidence of a complete world model. |
...
| M34-C03 | Let h-star(n) be the true least remaining cost in the stated model. In the module’s stated nonnegative convention, record \(0\le h(n)\le h^*(n)\); the upper inequality is **admissibility**. It is **consistent** when \(h(n)\le c(n,n')+h(n')\) for every stated transition, with \(h(g)=0\) for a goal. | Define goal set, reachable states, cost convention, finite/infinite setting, whether negative costs are prohibited, heuristic domain, and duplicate/reopen policy. | From consistency, derive \(g(n)+h(n)\le g(n')+h(n')\) along a transition; then identify which A* graph-search closed-set argument relies on that monotonicity. A theorem card must name its exact A* variant. | A heuristic can be admissible yet inconsistent; a no-reopen implementation needs stronger conditions than merely “it seemed to work.” An overestimating heuristic may find a route quickly but forfeits the claimed optimality guarantee. |
...
| M34-C10 | A first-order clause and resolution step can derive a consequence under explicit quantifier scope, substitution, equality, and premise conditions; failure to find a refutation in bounded search is not disproof. | Term language, variable/constant distinction, clause normalization, unifier, resolution rule, query negation, duplicate/termination policy, and domain assumptions. | Normalize an implication, unify a named constant, derive one resolvent, and distinguish “not proved” from “disproved.” | A variable capture, reversed implication, reused existential witness, or bounded search cutoff changes the claim; a formal proof does not establish sensor truth. |
| M34-C11 | A Bayesian network factorizes a declared joint law and an HMM filters a time-indexed hidden state under transition/emission assumptions; neither is automatically a causal model or a policy. | DAG/parent set, conditional-independence claim, initial law, transition/emission matrices, observation order, normalization, and evidence boundary. | Compare a factorization and one forward update; change a hidden-state assumption and identify which posterior changes. | Marginalizing a shared hidden cause can create dependence; normalized arithmetic can still answer the wrong model. |
| M34-C12 | A POMDP policy maps belief states to actions under hidden-state transitions and observations, while game-theoretic equilibrium is a no-profitable-unilateral-deviation property of a declared payoff game. | POMDP tuple, belief update, observation timing, horizon/discount, action/reward model, player payoffs, best responses, equilibrium concept, and information structure. | Derive one belief update and one best response; label what is a filtering result, policy result, or equilibrium result. | Repeating a posterior selector does not create a POMDP; minimax does not solve every general-sum game; an equilibrium is not fairness or authority. |
...
| **M34-S04 — Planning/CSP limits: encoding, reductions, and solver boundaries** (**M33**) | S34-03–S34-05, S34-09, S34-10, S34-16, S34-19 | Give a tiny planning/CSP encoding and a claimed reduction. Ask the learner to find the missing transformation direction or input-size assumption, then distinguish model-false, missing observation, unmodelled factor, timeout, infeasible, and unknown cases. | **CSP-limit claim card.** It does not infer practical impossibility, unsatisfiability, model-falsehood, or a M33 prerequisite completion from one code run. |
...
4. **No tested learner studio exists.** A local deterministic fixture/test now
   makes a two-world observation countermodel, two frontier-policy decisions,
   one A-star reopen counterexample, one AC-3 predecessor-requeue trace, one
   binary-relaxation status, and one two-stage Bellman card inspectable without
   accepting learner code or acting as a general solver. It is not a
   browser-accessible search/CSP/decision studio with a full input allowlist,
   accessibility review, and failure-mode documentation. A future studio must
   not execute arbitrary code or access undeclared network, package,
