...
| S32-12 | [PyTorch CUDA semantics](https://docs.pytorch.org/docs/stable/notes/cuda.html), [autograd mechanics](https://docs.pytorch.org/docs/stable/notes/autograd.html), and [torch.compile](https://docs.pytorch.org/docs/stable/generated/torch.compile.html) | PyTorch contributors | GPU work is ordinarily asynchronous; a named non-default-stream explanation needs both an inter-stream dependency and storage-lifetime handling, while reverse-mode history, graph breaks, dynamic shapes, and private saved-tensor details constrain the rest of the execution explanation. Stream/lifetime wording rechecked 2026-08-03. | [PyTorch BSD-3-Clause license](https://github.com/pytorch/pytorch/blob/main/LICENSE). Link-only/original examples; pin framework, CUDA/ROCm, driver, compiler, and device. |
...
| S32-15 | PyTorch [automatic mixed precision](https://docs.pytorch.org/docs/stable/amp.html) guide and [AMP examples](https://docs.pytorch.org/docs/stable/notes/amp_examples.html) | Mixed precision is a policy over compute/accumulation formats, scaling, non-finite detection, skipped updates, and backend support—not a portable dtype toggle. | Accessed 2026-08-04. PyTorch BSD-3-Clause; link-only/original paraphrase. Pin framework/backend/device and retain a reference dtype/oracle; do not copy settings, code, or benchmark claims. |
| S32-16 | PyTorch [DistributedDataParallel](https://docs.pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html) reference and [distributed training tutorial](https://docs.pytorch.org/tutorials/intermediate/ddp_tutorial.html) | Data partitioning, replica version, gradient reduction, collective completion, effective batch, straggler, and failure semantics are part of a distributed-training claim. | Accessed 2026-08-04. PyTorch BSD-3-Clause; link-only/original paraphrase. Code-reading and architecture reasoning only; no cluster, credential, or unbounded process lab is implied. |
...
| M32-C13 | A mixed-precision path is a declared numerical policy spanning compute dtype, accumulation/master state, scaling, and non-finite handling. | Framework/backend/device, per-operation dtype policy, accumulation order, scale schedule, skipped-update rule, reference dtype, tolerance, and semantic oracle. | A small finite match does not prove long-run stability, no skipped updates, or portable behavior; precision changes can alter reduction order and overflow/underflow. |
| M32-C14 | Distributed data parallelism changes the algorithm through data partitioning, collective reduction, synchronized parameter versions, and failure/straggler rules. | Rank/world definition, sampler and no-duplication evidence, reduction operator, effective batch, collective boundary, timeout/retry policy, communication cost, and global quality oracle. | A single rank's loss or a faster step does not prove global convergence, equivalent data, fair comparison, or reproducibility; a missing collective may be independent local training or stale replicas. |
...
| **M32-S05 — Autodiff, mixed precision, and optimization traces** (M28, M31) | S32-10–S32-12, S32-15 | Read a small original scalar loss trace: values, shapes, dtype, graph/transform boundary, gradient, precision policy, and a finite-difference check. Require the learner to name the claim that remains unproved after matching gradients. | **Autodiff-execution trace with a precision-policy row.** It does not turn a framework derivative into objective validity, convergence, or numerical-stability proof. |
| **M32-S06 — Reproducible distributed-accelerator systems architecture defense** (M12, M17, M19, M24, M28, M31) | S32-01–S32-16 | Assemble the prior artifacts into a deliberately narrow architecture-defense packet, including data partition, collective, mixed-precision, failure, and communication fields. The oral discussion asks what could change the result without changing the source code and what measurement would most reduce uncertainty. | **Scientific Python & Accelerators Dossier and learner-controlled oral-defense summary.** It is an evidence record, not a pass/fail exam, deployment approval, or portability guarantee. |
