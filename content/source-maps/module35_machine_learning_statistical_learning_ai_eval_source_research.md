...
| **M35-S02 — Problem formulation and classical baselines before learned models** (**M28, M34**) | S35-03, S35-04 | Present a narrow problem statement and an AI-generated model proposal. Ask for a rule/constraint/dummy/simple-linear baseline, exact information availability, and a predicted failure mode before revealing the candidate learned model. | **Classical-learning baseline comparison.** It does not claim a complex model is needed, a benchmark win is meaningful, or a baseline is operationally acceptable. |
...
| **M35-S04 — Learning dynamics: objectives, autodiff, optimization, and training systems** (**M28, M31, M32**) | S35-09, S35-10, S35-03 | Read an original scalar loss trace: data/representation, loss, gradient, update, dtype, seed, and observed validation result. Predict whether a finite-difference check or loss decrease validates a named stronger claim. | **Objective–optimization–generalization trace and reproducibility card.** They do not establish convergence, portability, generalization, or a correct objective. |
