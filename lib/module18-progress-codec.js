...
export function persistModule18Progress(storage, record) {
  if (!hasMeaningfulModule18Progress(record)) {
    // A context change can clear the final gate after an earlier save. Remove
    // that stale evidence rather than letting a reload resurrect it.
    clearModule18Progress(storage);
    return false;
  }
  if (
