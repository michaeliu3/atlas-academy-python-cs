...
const modelSelectionRelation = Object.freeze({
  id: "m35-constructed-selection-evidence-relation-v1",
  trainAndValidationLabelRule: "label = context",
  freshEvaluationLabelRule: "label = signal",
  shiftType: "declared conditional / label-relation shift",
  scope:
    "This is a separate constructed selection-evidence fixture. Its partition-specific label rules are not samples from the signal-routing story's label = Number(signal === context) relation.",
});
...
export function m35FitSelectFreshEvaluationTrace() {
  ...
  return {
    ...
    dataRelation: { ...modelSelectionRelation },
    partition: {
...
/**
 * Make one fully declared input-mixture shift inspectable for M35 without
 * treating it as robustness or deployment evidence.
 */
export function m35DeclaredRelationShiftProbe() {
  const source = relationEvidence("source-balanced");
  const shifted = relationEvidence("context-heavy");
  return {
    id: "m35-s03-declared-input-mixture-shift-probe",
    fixedPredictor: "signal-only: prediction = signal",
    fixedLabelRelation: "label = Number(signal === context)",
    changedPremise:
      "The input mixture changes from source-balanced to context-heavy while the fixed label relation remains unchanged.",
    source,
    shifted,
    conclusion:
      "For this fixed predictor and these two fully declared synthetic relations, expected accuracy changes from 0.50 to 0.75. The calculation identifies neither robustness, a causal mechanism, nor behavior under an undeclared future relation.",
    truthBoundary:
      "This deterministic relation comparison is not a sampled evaluation, IID evidence, or a generalization theorem. It is not a robustness result, a deployment forecast, or a decision recommendation.",
  };
}
