...
import courseGraph from "../content/course/course-graph.v1.json" with { type: "json" };
...
export const requiredFoundationAreas = [
  ...
  },
  {
    ...
    extension: {
      ...
      status: "authoring-only",
      note:
        "The advanced extension is still authoring-only. Rebuild the published calculus and linear-algebra foundations first; this diagnostic does not unlock it.",
    },
  },
  {
    id: "ai-ml",
    ...
    extension: {
      ...
      status: "authoring-only",
      note:
        "The advanced extension is still authoring-only. Use the published probability, calculus, and representation foundations; this diagnostic does not make the extension available.",
    },
  },
];
...
const publishedDiagnosticRouteModuleNumbers = new Set(
  courseGraph.modules
    .filter(({ availability, lifecycle }) => availability === "published" && lifecycle === "published")
    .map(({ number }) => number),
);
/** @type {Set<string>} */
...
export function toLearningBrief(attempt) {
  ...
  if (result.bridgeRecommendations.length === 0) {
    ...
    for (const recommendation of result.bridgeRecommendations) {
      lines.push(
        `- ${recommendation.area.label}: Q${String(recommendation.questionNumber).padStart(2, "0")} → published Module ${recommendation.route.moduleNumber} — ${recommendation.route.section}: ${recommendation.route.href}` +
...
export function validateDiagnosticDefinition() {
  ...
  for (const question of diagnosticQuestions) {
    ...
    if (!publishedDiagnosticRouteModuleNumbers.has(question.route.moduleNumber)) {
      errors.push(`${question.id} must route only to a published foundation`);
    }
