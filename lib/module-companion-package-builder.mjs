...
function privateGuidedStudyContext(courseModule) {
  ...
  if (!isPlainObject(study) || study.status !== "ready") {
    throw new Error(`${moduleLabel(courseModule)} privateGuidedStudy must be a ready study pack.`);
  }
  const workbookPath = text(study.workbookPath, `${moduleLabel(courseModule)} privateGuidedStudy.workbookPath`);
  ...
  return { status: "ready", workbookPath };
}
...
function privateGuidedStudySentence(privateGuidedStudy) {
  ...
  return `Private guided-study status: the portal remains hidden and authoring-only. The checked-in pack at ${privateGuidedStudy.workbookPath} is ready for direct designated-chat study. It does not create reader access, Core credit, a release, or a mastery claim.`;
}
