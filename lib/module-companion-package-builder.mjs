...
function recordSentence(recordBoundary) {
  return `A portable copied prompt begins in “${recordBoundary.portableStartupMode}” mode. In my learner-designated Teaching Assistant or Study Partner chat, the configured private Notion record is authorized to use “${recordBoundary.designatedChatMode}”: automatically create at most one concise note after a substantive learning conversation, not after every exchange. Never save raw voice, full transcripts, credentials, sensitive personal data, or off-record material. Honor pause, off-record, correction, and deletion requests, and say plainly if a requested write did not occur. Do not claim a saved note without direct evidence of the successful write.`;
}
