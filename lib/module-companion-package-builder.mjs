...
const requiredUnavailableWriteBehavior = "state-write-unverified-and-provide-ready-to-paste-summary";
const requiredDeletionUnavailable =
...
function privateGuidedStudyContext(courseModule) {
  const study = courseModule.state.privateGuidedStudy;
  if (study === undefined) return null;
  if (
    !isPlainObject(study) ||
    study.status !== "ready" ||
    study.delivery !== "designated-codex-chats" ||
    Object.keys(study).length !== 2 ||
    !Object.hasOwn(study, "status") ||
    !Object.hasOwn(study, "delivery")
  ) {
    throw new Error(`${moduleLabel(courseModule)} privateGuidedStudy must contain the ready status and designated-chat delivery only.`);
  }
  if (courseModule.state.availability !== "authoring-only") {
    throw new Error(`${moduleLabel(courseModule)} private guided study must not change portal availability.`);
  }
  return { status: "ready", delivery: "designated-codex-chats" };
}
...
function workflowRules(liveWorkflow) {
  ...
  if (notionSessionNotes.onUnavailable !== requiredUnavailableWriteBehavior) {
    throw new Error("Live Codex unavailable-write behavior must provide the ready-to-paste summary.");
  }
  ...
  if (
    ...
    !Array.isArray(unavailableNoteTemplate.fields) ||
    unavailableNoteTemplate.fields.length !== requiredUnavailableNoteTemplate.fields.length ||
    !requiredUnavailableNoteTemplate.fields.every(
      (field, index) => unavailableNoteTemplate.fields[index] === field,
    ) ||
    unavailableNoteTemplate.privacyReminder !== requiredUnavailableNoteTemplate.privacyReminder
  ) {
    throw new Error("Live Codex workflow must preserve the ready-to-paste unavailable-write note template.");
  }
  ...
  if (
    !isPlainObject(notionSessionNotes.learnerControlAcknowledgements) ||
    notionSessionNotes.learnerControlAcknowledgements.deletionUnavailable !== requiredDeletionUnavailable
  ) {
    throw new Error("Live Codex workflow must preserve the unverified-deletion acknowledgement.");
  }
  return {
    ...
    recordBoundary: {
      portableStartupMode,
      designatedChatMode,
      closurePhrase: recordingAuthorization.closurePhrase,
      renewalRule: recordingAuthorization.renewalRule,
    },
    recordPolicy: {
      reachabilityCondition: requiredNotionReachability,
      unavailableWriteBehavior: requiredUnavailableWriteBehavior,
      unavailableNoteTemplate,
      deletionUnavailable: requiredDeletionUnavailable,
    },
  };
}
...
function graphContext(courseModule, graphModules) {
  ...
  const academicPrerequisites = courseModule.academicPrerequisiteNumbers.map((number) => {
    ...
    return {
      ...
      privateGuidedStudy: privateGuidedStudyContext(prerequisite),
    };
  });
  ...
  return {
    ...
    declaredForwardHandoff: forward === null
      ...
      : {
        ...
        privateGuidedStudy: privateGuidedStudyContext(forward),
      },
  };
}
...
function availabilitySentence(availability, privateGuidedStudy = null) {
  switch (availability) {
    case "legacy-open":
    ...
    case "authoring-only":
      if (privateGuidedStudy?.status === "ready") {
        return "hidden authoring-only portal material; its designated private guided-study pack is ready for direct chat-led study, without reader access, Core credit, release, or a mastery claim";
      }
...
function forwardSentence(declaredForwardHandoff) {
  ...
  return `Canonical forward handoff: M${declaredForwardHandoff.number} (${declaredForwardHandoff.title}; ${availabilitySentence(declaredForwardHandoff.availability, declaredForwardHandoff.privateGuidedStudy)}). Carry forward the model, uncertainty, and smallest evidence artifact—not a claim of mastery.`;
}

function privateGuidedStudySentence(privateGuidedStudy) {
  if (privateGuidedStudy === null) return "";
  return "Private guided-study status: the portal remains hidden and authoring-only. An approved private pack is ready for direct designated-chat study through the private guided route. It does not create reader access, Core credit, a release, or a mastery claim.";
}
...
function recordSentence(recordBoundary, recordPolicy) {
  const fallback = recordPolicy.unavailableNoteTemplate;
  const fallbackFields = fallback.fields.map((field) => `- ${field}`).join("\n");
  return `A portable copied prompt begins in “${recordBoundary.portableStartupMode}” mode. In my learner-designated Teaching Assistant or Study Partner chat, the configured private Notion record is authorized to use “${recordBoundary.designatedChatMode}”: automatically create at most one concise note after a substantive learning conversation, not after every exchange. Automatic note behavior is possible only when the chat is designated, ${recordPolicy.reachabilityCondition}, the learning conversation is substantive, and records are neither paused nor off-record. Before a current substantive session can create an automatic note, I must confirm “records on” in that designated chat. ${recordBoundary.renewalRule} If I say “${recordBoundary.closurePhrase}”, close automatic session-summary authority: do not automatically create or update a further session note unless I later say “records on” in a new substantive session. An explicit correction or deletion request remains separately learner-authorized. Treat a session as substantive only when it names a module or learning topic, includes learner reasoning, a concrete evidence artifact, a misconception, or a counterexample, and ends with a learner-controlled next action or cross-role handoff. Never save raw voice, full transcripts, credentials, sensitive personal data, or off-record material. If that destination is unavailable or a result cannot be verified, say plainly that no Notion write is verified and provide this ready-to-paste local summary in chat:\n\n${fallback.title}\n${fallback.intro}\n${fallbackFields}\n${fallback.privacyReminder}\n\nHonor end session, pause, off-record, correction, and deletion requests. A correction or deletion may be reported as complete only after direct evidence. ${recordPolicy.deletionUnavailable} Do not claim a saved note without direct evidence of the successful write.`;
}
...
export function buildModuleCompanionPackage({
  ...
  const currentPrivateGuidedStudy = privateGuidedStudyContext(courseModule);
  const prerequisites = prerequisiteSentence(academicPrerequisites);
  const forward = forwardSentence(declaredForwardHandoff);
  const privateStudy = privateGuidedStudySentence(currentPrivateGuidedStudy);
  ...
  return {
    ...
    module: {
      ...
      privateGuidedStudy: currentPrivateGuidedStudy,
      academicPrerequisites,
