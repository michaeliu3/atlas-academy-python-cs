...
export function extractSessionLaunches(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const sessionMatch = lines[index].match(
      /^##\s+(?:(\d+)\.\s+)?Session\s+([1-6])\s+—\s+(.+?)\s*#*\s*$/u,
    );
    if (!sessionMatch) continue;

    const number = Number(sessionMatch[2]);
    const title = headingText(sessionMatch[3]);
    const fullTitle = headingText(
      lines[index].replace(/^##\s+/u, "").replace(/\s*#*\s*$/u, ""),
    );
