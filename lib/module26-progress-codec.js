 ...
 * malformed present v2 value is discarded and fails closed: the current and
 * legacy records are best-effort cleared, and it is never a cue to revive
 * older data. A meaningful valid legacy record is
...
export function restoreModule26Progress(storage) {
  ...
  const rawLegacy = storage.getItem(MODULE26_LEGACY_PROGRESS_STORAGE_KEY);
  const legacyRecord = parseModule26LegacyProgress(rawLegacy);
  if (!legacyRecord) {
    if (rawLegacy !== null) {
      try {
...
export function persistModule26Progress(storage, record) {
  if (!hasModule26MeaningfulProgress(record)) {
    clearModule26Progress(storage);
    return false;
  }
  if (
