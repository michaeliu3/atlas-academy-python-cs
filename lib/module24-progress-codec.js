...
export function restoreModule24Progress(storage) {
  ...
  if (currentRecord) {
    ...
    try {
      ...
      // Browser storage is optional; retain no in-memory blank progress.
    }
    try {
      storage.removeItem(MODULE24_LEGACY_PROGRESS_STORAGE_KEY);
    } catch {
      // Do not let a stale legacy record revive after a blank current shell.
    }
    return null;
  }
  if (rawCurrent !== null) {
  ...
  const legacyRecord = parseModule24LegacyProgress(
    storage.getItem(MODULE24_LEGACY_PROGRESS_STORAGE_KEY),
  );
...
export function persistModule24Progress(storage, record) {
  if (!hasModule24MeaningfulProgress(record)) return false;
  storage.setItem(
    MODULE24_PROGRESS_STORAGE_KEY,
