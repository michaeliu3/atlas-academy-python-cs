...
/**
 * Persist only meaningful prediction evidence. A blank in-memory grid must
 * remove any earlier bounded record instead of allowing an old answer to
 * reappear after a reload. The retired v2 generation is also removed so this
 * module never retains its former broad simulator snapshot.
 *
 * @param {{
 ...
 * @param {unknown} record
 */
export function persistModule19Progress(storage, record) {
  if (!hasMeaningfulModule19Progress(record)) {
  ...
  if (
    !storageOperationSucceeded(
      storage.setItem(
        ...
        module19ProgressCodec.serialize(record),
      ),
    )
  ) {
    return false;
