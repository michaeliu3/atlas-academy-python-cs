/**
 * A deliberately small, deterministic M34 authoring fixture for Sessions 1,
 * 2, 3, and 5. It exposes a state/observation countermodel, a frontier-policy
 * boundary, a relaxation result that must be checked in the original model,
 * one A-star reopen counterexample, one AC-3 requeue trace, and one two-stage
 * Bellman backup. It has no learner route, persistence,
...
/**
 * Return two declared world states with the same noisy observation. It makes
 * visible why an observation cannot be silently substituted for the world
 * state or a belief state. This is one finite countermodel, not a sensor
 * model, inference routine, or general model checker.
 */
export function m34ObservationBoundaryCard() {
  const observation = Object.freeze({ keyRackLight: "lit" });
  const worlds = Object.freeze([
    Object.freeze({
      id: "key-present-light-lit",
      worldState: Object.freeze({ room: "Entry", keyAtRack: true, hasKey: false, energy: 2 }),
      observation,
      takeKeyLegal: true,
    }),
    Object.freeze({
      id: "key-absent-light-lit",
      worldState: Object.freeze({ room: "Entry", keyAtRack: false, hasKey: false, energy: 2 }),
      observation,
      takeKeyLegal: false,
    }),
  ]);
  ...
  return {
    id: "m34-s01-observation-versus-world-card",
    observation,
    worlds,
    sameObservationDifferentLegalAction: true,
    finiteModelCheck: {
      proposition: "light-lit -> key-at-rack",
      evaluations: worlds.map((world) => ({
        worldId: world.id,
        lightLit: world.observation.keyRackLight === "lit",
        keyAtRack: world.worldState.keyAtRack,
        propositionTrue:
          world.observation.keyRackLight !== "lit" || world.worldState.keyAtRack,
      })),
      validInDeclaredWorldSet: false,
      countermodel: {
        worldId: "key-absent-light-lit",
        reason: "The light is lit while the key is absent, so the implication is false.",
      },
    },
    beliefBoundary:
      "A belief state would need a declared prior, observation likelihoods, and update rule; this card supplies none.",
    truthBoundary:
      "This card compares two declared synthetic worlds and evaluates one propositional implication. It is not a belief-state updater, sensor-validation result, general model checker, or evidence that the archive model captures a real environment.",
  };
}

/**
 * Return one fixed AC-3 requeue cascade. It records a predecessor arc only
 * because the domain of its neighbor changed; it does not execute AC-3 over
 * arbitrary variables, domains, or constraints.
 */
export function m34Ac3RequeueCard() {
  return {
    id: "m34-s03-ac3-requeue-card",
    constraints: Object.freeze(["A < B", "B < C"]),
    initialDomains: Object.freeze({ A: [1, 2], B: [1, 2], C: [2] }),
    steps: Object.freeze([
      Object.freeze({
        queueBefore: ["B->C"],
        processedArc: "B->C",
        removed: Object.freeze({ variable: "B", values: [2] }),
        domainsAfter: Object.freeze({ A: [1, 2], B: [1], C: [2] }),
        reEnqueuedPredecessorArcs: ["A->B"],
        queueAfter: ["A->B"],
      }),
      Object.freeze({
        queueBefore: ["A->B"],
        processedArc: "A->B",
        removed: Object.freeze({ variable: "A", values: [1, 2] }),
        domainsAfter: Object.freeze({ A: [], B: [1], C: [2] }),
        reEnqueuedPredecessorArcs: [],
        queueAfter: [],
        emptyDomain: "A",
      }),
    ]),
    conclusion:
      "The declared branch is inconsistent because no A value remains below the only B value; backtrack instead of treating local propagation as a complete solver.",
    truthBoundary:
      "This fixed two-step trace demonstrates one predecessor requeue and empty-domain outcome. It is not a general AC-3 implementation, a complete CSP solver, or a proof about another constraint graph.",
  };
}
...
export function m34TwoStageMdpBackupCard() {
  ...
  return {
    ...
    observationModel: {
      terminalObservation:
        "After inspect, the observation becomes `clear` or `blocked` before the terminal action.",
      terminalPolicyRequiresObservedSuccessor: true,
      hiddenSuccessorBoundary:
        "If the terminal state stayed hidden, this state-contingent continuation would require a belief-state model and a different backup.",
    },
    bellmanBackup:
