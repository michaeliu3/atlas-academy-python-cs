...
function startMermaidFence(line) {
  const match = line.match(openingFencePattern);
  if (!match) return null;
  const language = match[2].trim().split(/\s+/u)[0]?.toLowerCase();
  return language === "mermaid" ? { marker: match[1][0], length: match[1].length } : null;
}
...
/**
 * Enumerate Mermaid fences in a Markdown document. This is intentionally a
 ...
 * reports.
 */
export function scanMermaidBlocks(markdown, { sourcePath = "(unknown Markdown source)" } = {}) {
  ...
  for (const [index, line] of lines.entries()) {
    if (opening === null) {
      const candidate = startMermaidFence(line);
      if (candidate) {
        opening = candidate;
        sourceLines = [];
        startLine = index + 1;
  ...
  if (opening !== null) {
    blocks.push({
      ...
      metadata: null,
      errors: ["has an unclosed Mermaid fence"],
    });
  }
...
export function mermaidAccessibilityMetadataErrors(block) {
  ...
  if (block.renderSource === "") {
    errors.push("must retain Mermaid render source after metadata comments are removed");
  }
  return errors;
}
