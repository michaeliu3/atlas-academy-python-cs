...
test("M34 primary-source research remains a bounded authoring input, not a source-map, release, or publication claim", async () => {
  ...
  assert.match(research, /M34-C10.*first-order clause/u);
  assert.match(research, /M34-C11.*Bayesian network/u);
  assert.match(research, /M34-C12.*POMDP policy/u);
  assert.match(candidateLedger, /\\| S34-22 \\|/u);
